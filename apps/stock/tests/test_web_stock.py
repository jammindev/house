...
def test_stock_list_page_renders_with_props(client, user, household, membership):
    ...
    response = client.get(reverse("app_stock"), HTTP_X_HOUSEHOLD_ID=str(household.id))
    ...
    assert props["newUrl"] == reverse("app_stock_new")
...
def test_stock_detail_and_edit_pages_render(client, user, household, membership, zone, category, item):
    ...
    detail_response = client.get(
        reverse("app_stock_detail", kwargs={"item_id": item.id}),
        HTTP_X_HOUSEHOLD_ID=str(household.id),
    )
    assert detail_response.status_code == 200
    assert "core/react_page.html" in [template.name for template in detail_response.templates]
    detail_props = detail_response.context["react_props"]
    assert detail_props["itemId"] == str(item.id)

    edit_response = client.get(
        reverse("app_stock_edit", kwargs={"item_id": item.id}),
        HTTP_X_HOUSEHOLD_ID=str(household.id),
    )
    assert edit_response.status_code == 200
    assert "core/react_page.html" in [template.name for template in edit_response.templates]
    form_props = edit_response.context["react_props"]
