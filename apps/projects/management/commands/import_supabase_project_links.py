...
SOURCE_PROJECT_ZONES_SQL = """
SELECT project_id, zone_id, created_at, created_by
FROM public.project_zones
"""

SOURCE_PROJECT_DOCUMENTS_SQL = """
SELECT project_id, document_id, role, note, created_at, created_by
...
class Command(BaseCommand):
    help = "Import project_zones and project_documents links from Supabase into local Django DB."
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            help="Only links where project belongs to this household are imported.",
        )
        parser.add_argument(
            "--fallback-user-id",
            type=int,
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.NOTICE("Fetching project link rows from Supabase..."))
        zone_rows = self._fetch_source_rows(dsn, SOURCE_PROJECT_ZONES_SQL)
        document_rows = self._fetch_source_rows(dsn, SOURCE_PROJECT_DOCUMENTS_SQL)
        self.stdout.write(
            self.style.NOTICE(
                f"Fetched {len(zone_rows)} project_zones and {len(document_rows)} project_documents."
            )
        )
        ...
        with transaction.atomic():
            zones_counts = self._import_project_zones(
                rows=zone_rows,
            ...
            documents_counts = self._import_project_documents(
                rows=document_rows,
                target_household_id=target_household_id,
                fallback_user_id=fallback_user_id,
                existing_user_ids=existing_user_ids,
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"project_zones import: created={zones_counts.created}, updated={zones_counts.updated}, skipped={zones_counts.skipped}"
            )
        )
        self.stdout.write(
            self.style.SUCCESS(
                f"project_documents import: created={documents_counts.created}, updated={documents_counts.updated}, skipped={documents_counts.skipped}"
            )
        )
    ...
    def _import_project_zones(
        ...
        for row in rows:
            ...
            exists = ProjectZone.objects.filter(project_id=project_id, zone_id=zone_id).exists()
            if dry_run:
                if exists:
                    counters.updated += 1
                else:
            ...
            obj, created = ProjectZone.objects.get_or_create(
            ...
            if not created:
                ProjectZone.objects.filter(pk=obj.pk).update(
                    created_by_id=self._map_user_id(row.get("created_by"), fallback_user_id, existing_user_ids)
                )
            if row.get("created_at") is not None:
                ProjectZone.objects.filter(pk=obj.pk).update(created_at=row["created_at"])
