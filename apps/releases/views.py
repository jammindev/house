...
Réservé au **staff/superuser Django** (`IsAdminUser`) : c'est de l'info de
développement de la plateforme (ce qui a été livré), pas de la donnée foyer, et
elle est identique pour tous. Aucune écriture via l'API — les entrées sont
générées par la command ``generate_changelog`` au déploiement.
"""
