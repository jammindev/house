...
def create_tracker(
    ...
    *,
    ...
    Reuses ``TrackerSerializer`` so validation (project household scope) stays
    in one place.
    """
...
def update_tracker(household, user, tracker: Tracker, *, fields: dict) -> Tracker:
    ...
    allowed = {'name', 'unit', 'description', 'emoji', 'is_active', 'project'}
...
def delete_entry(household, user, entry: TrackerEntry) -> None:
    ...
    This is also the undo path for agent- and quick-add-created entries.
    """
...
def refresh_tracker_cache(tracker: Tracker) -> None:
    ...
    Always recomputed, never incremented — so they cannot drift.
    """
...
def build_entries_summary(tracker: Tracker) -> str:
    ...
    One line per entry (newest first) with the delta vs the previous entry.
    """
    entries = list(
