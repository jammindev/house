...
type RawInteraction = {
    ...
    interaction_tags?: { tag?: any | null }[] | null;
    interaction_contacts?: { contact?: any | null }[] | null;
...
function mapInteraction(raw: RawInteraction): Interaction {
    const project = raw.project
        ? {
            ...
            status: (raw.project.status ?? "draft") as any,
        }
        : null;

    const tags = raw.interaction_tags?.map((entry) => entry?.tag).filter(Boolean) ?? [];

    const contacts = raw.interaction_contacts
    ...
    const structures = raw.interaction_structures
...
export function useStructureInteractions(structureId?: string | null, options?: { limit?: number }) {
    const { t } = useI18n();
    ...
    useEffect(() => {
        ...
        const load = async () => {
            ...
            try {
              ...
              interaction_structures:interaction_structures!inner(
                structure:structures(
                  id,
                  name,
                  type
            ...
            `
                    ...
                    .eq("interaction_structures.structure_id", structureId)
                    .order("occurred_at", { ascending: false })
                    .limit(limit);

                if (interactionsError) throw interactionsError;

                const mapped = ((rows ?? []) as unknown as RawInteraction[]).map(mapInteraction);
