-- 1) Refresh create_interaction_with_zones to support quote interactions with metadata ---------
do $$
begin
  if exists (select 1 from pg_proc where proname = 'create_interaction_with_zones') then
    execute 'drop function create_interaction_with_zones(uuid, text, uuid[], text, text, text, timestamptz, uuid[], uuid[], uuid[], uuid)';
  end if;
end $$;
...
declare
  ...
  v_allowed_types constant text[] := ARRAY['note','todo','call','meeting','document','expense','message','signature','other','quote'];
...
-- 2) Extend interactions.type check constraint to allow quotes ----------------
do $$
begin
