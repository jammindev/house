...
- **Frontend** : onglet Consommation (`ConsumptionTab.tsx`) — granularité + navigation de période (`useSessionState`), BarChart Recharts empilé par cadran (tokens `--chart-*`), dialogs compteur/relevé/import (preview + mapping générique), relevés récents avec undo. Recharts est entré au projet ici. Tarifs : `TariffsDialog.tsx` (liste des périodes ↔ formulaire dans un seul SheetDialog, action « Gérer les tarifs » sur la barre compteur) + bandeau € sous le total kWh (visible seulement si `total_cost_eur !== null`).
