...
- **Tests** : oui — architecture (`test_models.py`, `test_serializers.py`, `test_views.py`) + consommation (`test_services_consumption.py`, `test_views_consumption.py`, `test_services_imports.py`, `test_views_imports.py`, `test_agent_integration.py`) + tarifs (`test_services_tariffs.py`, `test_views_meter_tariffs.py`) + `factories.py` + E2E (`e2e/electricity.spec.ts`, `e2e/electricity-consumption.spec.ts`, `e2e/electricity-tariffs.spec.ts`)
- **Migrations** : 12 total
...
- Modèles consommation (parcours 10) : `ElectricityMeter` (tarification base/HP-HC, fuseau IANA du point de comptage, `export_url` = page fournisseur où l'export se télécharge), `MeterReading` (relevés d'index, monotonie validée), `MeterTariff` (période tarifaire : prix TTC €/kWh par cadran + abonnement €/mois, unique `(meter, valid_from)`), `ConsumptionRecord` (**pivot générique multi-pays** : Wh entiers, intervalle explicite en minutes, cadran, source `reading`/`import`), `ConsumptionImport` (trace d'audit des imports)
...
- Endpoints consommation : `/api/electricity/meters/`, `/meter-readings/`, `/meter-tariffs/` (`?meter=`), `GET /consumption/summary/?meter=&granularity=hour|day|month|year&date_from=&date_to=` (agrégation Postgres dans le fuseau du compteur), `/consumption/imports/` (+ `preview/`)
...
- **Coût € (lot 5, issue #218)** : le tarif applicable à un jour local = dernier `valid_from` ≤ jour (`_tariff_lookup`, bisect). Chaque jour est valorisé au tarif en vigueur, par cadran pour HP/HC. Abonnement proratisé au jour calendaire du mois réel (`monthrange`), borné à aujourd'hui, démarrant au premier tarif. **Aucun coût inventé** : jour/période sans tarif applicable → `cost_eur`/`energy_cost_eur`/`total_cost_eur` à `None`, jamais un zéro trompeur. Le summary expose `energy_cost_eur`, `subscription_cost_eur`, `total_cost_eur` + `cost_eur` par seau (€ arrondis à 2 décimales).
...
- **Frontend** : onglet Consommation (`ConsumptionTab.tsx`) — granularité + navigation de période (`useSessionState`), BarChart Recharts empilé par cadran (tokens `--chart-*`), dialogs compteur/relevé/import (preview + mapping générique, raccourci « Télécharger le fichier chez mon fournisseur » depuis `meter.export_url`, enregistrable directement dans le dialog d'import), relevés récents avec undo. Recharts est entré au projet ici. Tarifs : `TariffsDialog.tsx` (liste des périodes ↔ formulaire dans un seul SheetDialog, action « Gérer les tarifs » sur la barre compteur) + bandeau € sous le total kWh (visible seulement si `total_cost_eur !== null`).
...
- Docs : `docs/parcours/PARCOURS_10_ANALYSER_LA_CONSOMMATION_ELECTRIQUE.md` + `PARCOURS_10_BACKLOG_TECHNIQUE.md`. Coût € livré (lot 5, #218). V2 différée : #202 (comparaisons, sync auto, autres fluides).
