...
- [ ] Les dépenses liées à un projet (`Interaction` de type `expense`) stockent le montant dans `metadata` (JSONField non-structuré) : ajouter un champ `amount` décimal structuré pour permettre agrégation et validation — *source : issue #67 ; `apps/interactions/models.py:71-75`*
...
- [ ] Couvrir `pin`/`unpin` et les actions custom par des tests — `apps/projects/tests/` n'a pas de `test_models.py` ni `test_serializers.py` ni `factories.py` — *source : `apps/projects/tests/`*
