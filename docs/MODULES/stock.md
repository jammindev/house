...
> Audit : 2026-04-28 (maj 2026-07-23). Rôle : gérer l'inventaire des consommables et fournitures du foyer (catégories, quantités).
...
- Modèles principaux : `StockCategory` (nom, color, emoji, sort_order) ; `StockItem` (quantity, min/max, supplier ; `status` **dérivé**, non saisi) ; `StockLevelReading` (niveau daté absolu — socle de la courbe de conso, parcours 18)
...
- **Écritures via `apps/stock/services.py`** (parcours 18, lot 18.1) — source de vérité partagée view + agent : `purchase_stock_item` (recalage quantité, snapshots, relevés, interaction dépense, statut+notif), `record_inventory` (quantité absolue + relevé), `recompute_status` (statut **100% dérivé** de quantité/min : `out_of_stock` / `low_stock` / `in_stock`). L'action `purchase/` compose l'`Interaction` expense liée via la FK polymorphe (`create_expense_interaction`, `kind=stock_purchase`, avec `brand` en metadata).
...
- **Statut 100% dérivé** (2026-07-23) : `status` n'est plus saisi (read-only côté serializer, dérivé à la création et à chaque écriture de quantité via `recompute_status`). Les états `ordered`/`reserved`/`expired` ont été retirés de l'enum (data-migration de reclassement `0003`). Le champ `expiration_date` a été **entièrement retiré** (mal modélisé au niveau article ; à réintroduire plus tard au niveau lot/achat).
- Toute écriture de quantité par le CRUD (`perform_create`/`perform_update`) passe par la logique inventaire (relevé + recompute + notif) — cf. `#326`.
