- Créer `HouseholdListView` et `HouseholdDetailView` dans `apps/core/views.py`
  pour encapsuler le scoping household et éviter le boilerplate répété dans chaque vue.
  Voir `docs/REFACTO_HOUSEHOLD_LIST_VIEW.md` et `docs/REFACTO_HOUSEHOLD_DETAIL_VIEW.md`.
...
- Quand on est plus sur la page liste la side bar n'affiche plus sur quelle page on est
