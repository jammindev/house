...
export function useDeleteDocument({ shouldDeleteDocument = true }: { shouldDeleteDocument?: boolean } = {}) {
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState("");

  const deleteFile = useCallback(
    async (file: Pick<Document, "id" | "file_path" | "interaction_id">) => {
      setLoading(true);
      setError("");
      try {
    ...
    [shouldDeleteDocument] // ✅ dépendance ajoutée ici
  );
