// shared/src/hooks/useContacts.ts
...
import { getPrimaryEmail, getPrimaryPhone } from "../lib/contacts";
...
interface UseContactsProps {
    ...
    getSupabaseClient: () => Promise<any>; // Abstraction du client Supabase
    getErrorMessage: (error: unknown, fallback: string) => string; // Abstraction des messages d'erreur
}

export function useContacts({ householdId, getSupabaseClient, getErrorMessage }: UseContactsProps) {
    ...
    const reload = useCallback(async () => {
        ...
        try {
            ...
            const normalized = sortContacts((data ?? []).map((item: any) => normalizeContact(item as RawContact)));
    ...
    }, [householdId, getSupabaseClient, getErrorMessage]);

    const createContact = useCallback(
        async (input: CreateContactInput) => {
            if (!input.householdId) throw new Error(getErrorMessage(null, "Household ID required"));
            ...
            try {
                ...
                if (!userId) throw new Error(getErrorMessage(null, "User not found"));

                const firstName = input.firstName?.trim() ?? "";
                const lastName = input.lastName?.trim() ?? "";
                if (!firstName && !lastName) {
                    throw new Error(getErrorMessage(null, "Name is required"));
                }
                ...
                if (!contactId) throw new Error(getErrorMessage(null, "Failed to create contact"));
        ...
        [getSupabaseClient, getErrorMessage]
    );
    ...
    const updateContact = useCallback(
        async (input: UpdateContactInput) => {
            ...
            try {
                ...
                if (!existingContact) {
                    ...
                    if (existingError) {
                        if (isNotFoundError(existingError)) {
                            throw new Error(getErrorMessage(null, "Contact not found"));
                        }
                        throw existingError;
                    }
                    if (!existingData) throw new Error(getErrorMessage(null, "Contact not found"));
                ...
                if (!refreshedData) throw new Error(getErrorMessage(null, "Contact not found"));
        ...
        [contacts, getSupabaseClient, getErrorMessage]
    );
